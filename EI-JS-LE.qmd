...
##might be delated, because not helpful for my thesis purpose
...
\noindent \textit{Job Satisfaction}
...
\noindent \textit{Leader’s Emotional Intelligence}
...
\noindent \textit{Leadership Effectiveness}
...
plot1 <- ggplot(clean_data, aes(x = EI_Mean, y = LE_Mean)) + 
  ...
  labs(title = "Correlation between Leaders' Emotional Intelligence and Leadership Effectiveness", x = "Emotional Intelligence", y = "Leadership Effectiveness") +
  theme_minimal() +
...
plot2 <- ggplot(clean_data, aes(x = EI_Mean, y = JS_Mean)) + 
  ...
  geom_smooth(method = "lm", color = "blue", se = FALSE) +
  stat_cor(method = "pearson", color = "black", label.x = 3, label.y = 4) +
  labs(title = "Correlation between Leaders' Emotional Intelligence and Job Satisfaction", x = "Emotional Intelligence", y = "Job Satisfaction") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
plot3 <- ggplot(clean_data, aes(x = LE_Mean, y = JS_Mean)) + 
  ...
  geom_smooth(method = "lm", color = "blue", se = FALSE) +
  stat_cor(method = "pearson", color = "black", label.x = 3, label.y = 4) +
  labs(title = "Correlation between Leadership Effectiveness and Job Satisfaction", x = "Leadership Effectiveness", y = "Job Satisfaction") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) 
...
# Use forcats to reorder the variables by their mean scores
tidy_data$Variable <- fct_reorder(tidy_data$Variable, tidy_data$Score, .fun = "mean")

# Create the plot using ggplot
ggplot(tidy_data, aes(x = Variable, y = Score)) +
  geom_boxplot(aes(color = Variable), alpha = 0.7) + # Boxplot to show score distribution
  geom_jitter(aes(color = Variable), width = 0.1, alpha = 0.5) + # Jitter for individual data points
  facet_wrap(~Variable, scales = "free") + # Faceting by variable
  labs(title = "Correlation Between EI_Mean, LE_Mean, and JS_Mean", 
       x = "Variables", 
       y = "Mean Scores") +
  theme_minimal() +
  theme(legend.position = "none") # Hide legend as we're faceting
