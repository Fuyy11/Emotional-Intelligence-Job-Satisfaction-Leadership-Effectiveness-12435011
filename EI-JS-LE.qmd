...
# Save as a PNG
gtsave(normality_gt, "normality_table.png", expand = 20, vwidth = 1000, vheight = 600)

# Insert the image 
cat("\\vspace{10mm}") # Add 10mm space above
knitr::include_graphics("normality_table.png")
cat("\\vspace{10mm}") #Add 10mm space below

```
## Descriptive statistics
```{r}
#| label: 7_descriptive_statistics
#| message: false
#| warning: false
...
long_data <- clean_data[, c("EI_Mean", "LE_Mean", "JS_Mean")]
long_data <- reshape2::melt(long_data)

# Create pairwise scatter plot with correlation coefficients
ggplot(long_data, aes(x = value, y = variable)) +
  geom_point(aes(color = variable), size = 2) +
  stat_smooth(method = "lm", col = "red", se = FALSE) +
  stat_cor(aes(label = ..r.label..), color = "black", size = 5) +
  facet_wrap(~ variable, scales = "free") +
  theme_minimal() +
  labs(title = "Pairwise Scatter Plots with Correlation Coefficients", x = "Value", y = "Variable")
```
